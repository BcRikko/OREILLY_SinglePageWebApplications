...
spa.chat = (function () {
    ...
    // 引数：
    //   * position_type - enum('cloed', 'opened', または'hidden')
    //   * callback - アニメーションの最後のオプションのコールバック
    // このコールバックは単一引数としてスライダー div を表すjQueryコレクションを受け取る
    // 動作：
    // このメソッドはスライダーを要求された一に移動する
    ...
    onSubmitMsg = function (event) {
        ...
        jqueryMap.$input.focus();
        jqueryMap.$send.addClass('spa-x-slect');
        setTimeout(
    ...
    // 目的：初期化前にモジュールを構成する
    // 引数：
    //   * set_chat_anchor - オープンまたはクローズ状態を表すように
    //     URIアンカーを変更するコールバック。このコールバックは要求された状態を
    //     満たさない場合にはfalseを返さなければいけない
    ...
    //   ひとつのDOMコンテナを表すjQueryコレクション
    // 動作：
    //   指定されたコンテナにチャットスライダーを付加し、HTMLコンテナで埋める
    ...
    //   縮小しらウィンドウサイズに合わせてチャットスライダーのサイズを変更する。
    // 戻り値：boolean
    //   * false - リサイズを考慮していない
    //   * true - リサイズを考慮した
    // 例外発行：なし
